...
$this->params = ['breadcrumb'  => [
                                    ['name' => '消息管理','url' => '#','current' => 0],
                                    ['name' => '消息列表','url' => '#','current' => 1],
                                  ]
                ];
?>
<div class="row-fluid">
				...
				<tbody>
				   ...
				   <?php foreach ($models AS $k => $v): ?>
					<tr id="tr_<?= $v['id'] ?>">
						...
						<td><?= $v['create_time'] ?></td>
						<td>
						   <a href="<?= Url::to(['message/form','id' => $v['id'],'pid' => Yii::$app->controller->pid]);?>" class="btn btn-primary"><i class="icon-pencil icon-white"></i> 查看</a>
						</td>
					</tr>
					<?php endforeach; ?>
					<?php else: ?>
					<tr>
					   <td colspan="5">
   					   <div class="alert alert-info" style="margin-top:22px;">
   							...
   							<strong>友情提醒！</strong> 还没有人给此公众号发消息！
   						</div>
					   </td>
			      </tr>
					<?php endif; ?>
