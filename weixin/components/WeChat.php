...
use weixin\components\ErrorCode;
use weixin\components\PKCS7Encoder;
use weixin\components\Prpcrypt;
...
class WeChat extends Component
{
	...
	public function init()
	{
		...
		$this->encodingAesKey = isset($this->options['encodingaeskey'])?$this->options['encodingaeskey']:'';
		$this->appid = isset($this->options['appid'])?$this->options['appid']:'';
		$this->appsecret = isset($this->options['appsecret'])?$this->options['appsecret']:'';
		$this->debug = isset($this->options['debug'])?$this->options['debug']:false;
		$this->logcallback = isset($this->options['logcallback'])?$this->options['logcallback']:false;
	}
...
<Nonce><![CDATA[%s]]></Nonce>
	...
	protected function setCache($cachename,$value,$expired){
		...
		//直接改写
		if (Yii::$app->cache->set($cachename,$value,$expired)) {
		    return true;
		}else{
		    return false;
		}
	...
	protected function getCache($cachename){
		...
		//直接改写
		if ($value = Yii::$app->cache->get($cachename)) {
		    return $value;
		}else{
		    return false;
		}
	...
	protected function removeCache($cachename){
		...
		//直接改写
		if (Yii::$app->cache->delete($cachename)) {
		    return true;
		}else{
		    return false;
		}
